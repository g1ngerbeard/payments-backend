...
class InmemOrderRepository(failRate: Double = 0.0) extends OrderRepository[ProblemOr] {
  ...
  private def doCall[T](call: => T): ProblemOr[T] =
    Either.cond(isSuccessfulCall(failRate), call, StorageFailure("this is mocked failure"))
